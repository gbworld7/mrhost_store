...
export function Tabs({ tabs, active, onChange }) {
  return (
    <div style={{ display: "flex", gap: 22, borderBottom: `1px solid ${T.line}`, overflowX: "auto", padding: "0 2px" }}>
      {tabs.map((tb) => {
        const on = tb.key === active;
        return (
          <button key={tb.key} onClick={() => onChange(tb.key)} style={{ position: "relative", background: "none", border: "none", cursor: "pointer", padding: "13px 2px", whiteSpace: "nowrap", fontFamily: T.fontBody, fontSize: 15, fontWeight: on ? 700 : 500, color: on ? T.ink : T.gray2, transition: "color .2s" }}>
            {tb.label}
